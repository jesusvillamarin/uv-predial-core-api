...
module.exports.handler = async (event, context, callback) => {
    ...
    try {
        ...
        if(isNaN(+body.cfn)) throw new BadRequestException('UV.PREDIAL.MSG.21', { cfn: body.cfn });
        if(body.cfn.length != 8) throw new BadRequestException('UV.PREDIAL.MSG.22', { cfn: body.cfn });
        if(body.type.length != 1) throw new BadRequestException('UV.PREDIAL.MSG.23', { type: body.type });
        if(body.locality.length != 3) throw new BadRequestException('UV.PREDIAL.MSG.24', { locality: body.locality });
        if(body.region.length != 2) throw new BadRequestException('UV.PREDIAL.MSG.25', { region: body.region });
        if(body.block.length != 3) throw new BadRequestException('UV.PREDIAL.MSG.26', { block: body.block });
        if(body.lot.length != 3) throw new BadRequestException('UV.PREDIAL.MSG.27', { lot: body.lot });
        if(body.level.length != 2) throw new BadRequestException('UV.PREDIAL.MSG.28', { level: body.level });
        if(body.department.length != 3) throw new BadRequestException('UV.PREDIAL.MSG.29', { department: body.department });
